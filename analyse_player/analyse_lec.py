...
def load_data():
    """
    Finds the CSV file by checking both the parent directory (standard project structure)
    and the local directory.
    """
    file_name = "Australia2025_LEC_cleaned_sorted.csv"
    
    # Strategy: Get absolute path of this script, then look for the data folder
    script_dir = Path(__file__).parent.absolute()
    
    # Path 1: Check ../cleaned_Csv/ (One level up from script)
    path_option_1 = script_dir.parent / "cleaned_Csv" / file_name
    
    # Path 2: Check cleaned_Csv/ (Same level as script)
    path_option_2 = script_dir / "cleaned_Csv" / file_name

    if path_option_1.exists():
        target_path = path_option_1
    elif path_option_2.exists():
        target_path = path_option_2
    else:
        st.error(f"Could not locate {file_name}. Checked: {path_option_1}")
        return pd.DataFrame()
